...
import { buildUserFilters } from './filters';
import { API_RESOURCES, type ResultOptions } from '../../../helpers/Sharetribe.types';

/**
 * Fluent builder for building user query parameters
...
export class UserQueryBuilder extends BaseQueryBuilder {
	withFilters(filterOptions: IDataObject): this {
		const filters = buildUserFilters(filterOptions);
		if (filters.length > 0) {
	...
	withOptions(options: {
		...
		this.deferredApiResource = API_RESOURCES.USER;
		this.deferredFieldsKey = 'userFields';
